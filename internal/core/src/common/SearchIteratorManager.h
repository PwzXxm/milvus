...
private:
   ...
   GetOrPut(const SearchIteratorV2Info& search_info,
    ...
    bool UpdateLru(const std::string& token, const std::optional<int32_t> ttl) {
        if (!ttl.has_value()) {
            return true;
        }

        const std::chrono::seconds ttl_seconds = std::chrono::seconds(ttl.value());
